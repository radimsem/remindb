...
**Orientation** — `memory_tree` vs listing + reading every file:
...
**Search** — `memory_search` + `memory_fetch` vs grep + reading matched files:
...
**Fetch** — `memory_fetch` with token budget vs reading the whole file:
...
**Delta** — `memory_delta` vs re-reading the modified file:
