...
- `skills/efficient-memo/` — **public** skill shipped to MCP clients (distinct from `.claude/skills/`)
...
| How clients call the MCP tools (the contract) | `skills/efficient-memo/SKILL.md` |
...
The eight `Memory*` tools are a contract shipped to clients via `skills/efficient-memo/SKILL.md`. Renaming, removing, or changing semantics breaks every client and desyncs the public skill. Use the **`add-mcp-tool` skill** for any new/modified tool, follow `.claude/rules/mcp-tool-conventions.md`, and dispatch the **`mcp-surface-reviewer` agent** before merge.
...
`DecayRate`, `AccessBoost`, `ColdThreshold`, `NotifyThreshold`, `TickInterval` are documented numerically in `skills/efficient-memo/SKILL.md`. Changing any one shifts search ranking, the cold-set query, *and* the client notification stream — and the public skill drifts silently. Use the **`tune-temperature-policy` skill**.
...
- **Not** the MCP contract spec → `.claude/rules/mcp-tool-conventions.md` + `skills/efficient-memo/SKILL.md`.
