...
Reference for `memorize`. Load when authoring a **file for the compile plane** (see `references/write-paths.md`) and you need the exact block→node mapping or the automatic compaction rules behind a node's `token_count`.
