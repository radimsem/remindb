...
| [`remindb-setup/`](./remindb-setup/) | **Setup wizard.** Run `/remindb-setup` (interactive) or `/remindb-setup automode` (hands-off) to configure a workspace from inside a session: binary check + fallback install, author the `.remindb/` config, offer to reseed temperatures/pins onto existing nodes, and advise when a restart is needed. SKILL.md is the wizard spine; depth lives in [`remindb-setup/references/`](./remindb-setup/references/) (`config-model`, `config-examples`, `automode-playbook`). |
