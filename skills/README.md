...
| Skill | Purpose |
|---|---|
| [`remind/`](./remind/) | **Read path.** Orient with the tree, search/fetch under a token budget, resync via delta. Covers the node/snapshot/temperature mental model and FTS5 query syntax. |
...
The two are designed to be loaded together. `memoize` references the mental model that `remind` defines; install both.
